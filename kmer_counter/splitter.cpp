...
bool CSplitter::ProcessReads(uint64 id, uchar *_part, uint64 _part_size, ReadType read_type)
{
	part = _part;
	part_size = _part_size;
	part_pos = 0;
			...
			for (; i < seq_size; ++i)
			{
				if (seq[i] < 0)//'N'
				{
					if (len >= kmer_len)
					{
						...
						bins[bin_no]->PutExtendedKmer(id, seq + i - len, len);
					}
					len = 0;
					++i;
					break;
				...
				{
					bin_no = s_mapper->get_bin_id(current_signature.get());
					bins[bin_no]->PutExtendedKmer(id, seq + i + 1 - len, len);
					i -= kmer_len - 2;
					len = 0;
					break;
				}
...
void CWSplitter::operator()()
{
    ...
    uint64 tmp_id;
		ReadType read_type;
		if (pq->pop(tmp_id, part, size, read_type))
		{			
			spl->ProcessReads(tmp_id, part, size, read_type);
			pmm_fastq->free(part);
		}
	}
	spl->Complete();
