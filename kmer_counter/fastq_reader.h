...
public:
	...
	uint64 read(uchar* buff, uint64 size, bool& last_in_file, uint64& ref_id);
	void IgnoreRest()
	{
		if (in_data)
		...
		while (binary_pack_queue->pop(in_data, in_data_size, file_part, compression_type, file_id))
		{
			if(in_data_size)
				pmm_binary_file_reader->free(in_data);
			in_data = nullptr;
...
public:
	...
	bool GetPartFromMultilneFasta(uchar *&_part, uint64 &_size, uint64& ref_id);
	
	void ProcessBam();	

	bool GetPart(uchar *&_part, uint64 &_size);

	bool GetPartNew(uchar *&_part, uint64 &_size, ReadType& read_type, uint64& ref_id);
	void Init()
	{
