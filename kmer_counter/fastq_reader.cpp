...
void CFastqReader::PreparePartForSplitter(uchar* data, uint64 size, uint32 id, uint32 file_no)
{
	...
	if (state.prev_part_size)
	{
		...
		if (part_queue)
		{
			part_queue->push((uint64)id, state.prev_part_data, state.prev_part_size, ReadType::na);
		}
	...
	while (true)
	{
		if (bpos == size) // this pack contains full records, may be passed 
		{
			...
			if (part_queue)
			{
				part_queue->push((uint64) id, data, size, ReadType::na);
			}
			else if (stats_part_queue)
			{
		...
		if (span_multiple_bgzf_blocks)
		{
			...
			if (part_queue)
			{
				part_queue->push((uint64)id, data, bpos, ReadType::na);
			}
			else if (stats_part_queue)
			{
...
bool CFastqReader::GetPartFromMultilneFasta(uchar *&_part, uint64 &_size, uint64& ref_id)
{
	uint64 readed = 0;
	...
	bool last_in_file;
	readed = data_src.read(part + part_filled, (part_size - 1) - part_filled, last_in_file, ref_id); //part_size - 1 to eventually append EOL if not present at EOF
...
FORCE_INLINE bool CFastqReader::GetNextSymbOfLongReadRecord(uchar& res, int64& p, int64& size)
{
	if (p == size)
	{
		bool last_in_file;
    uint64 refid_nouse;
		size = data_src.read(part, part_size - 1, last_in_file, refid_nouse);//part_size - 1 to eventually append EOL if not present at EOF
		if(last_in_file)
...
bool CFastqReader::GetPartNew(uchar *&_part, uint64 &_size, ReadType& read_type, uint64& ref_id)
{
	if (file_type == multiline_fasta)
	{
		read_type = ReadType::na;
		return GetPartFromMultilneFasta(_part, _size, ref_id);
	}
...
//----------------------------------------------------------------------------------
uint64 CFastqReaderDataSrc::read(uchar* buff, uint64 size, bool& last_in_file, uint64& ref_id)
{
	last_in_file = false;
	if (!in_progress)
	{
		if (!binary_pack_queue->pop(in_data, in_data_size, file_part, compression_type, ref_id))
		{
			end_reached = true;
			return 0;
		}
		...
		do
		{
			...
			{				
				...
				if (!multistream || garbage)
				{
					...
					in_progress = false;
					//pull end
					bool queue_end = !binary_pack_queue->pop(in_data, in_data_size, file_part, compression_type, ref_id);
					...
					if (garbage)
					{
						...
						while (tmpfilepart != FilePart::End)
						{
							...
							binary_pack_queue->pop(tmp, tmpsize, tmpfilepart, tmpcomptype, ref_id);
						}
					}
					last_in_file = true;
					break;
	...
	else if (compression_type == CompressionType::bzip2)
	{
		...
		do
		{
			if (!_bz_stram.avail_in)
			{
				pmm_binary_file_reader->free(in_data);
				in_data = nullptr;
				binary_pack_queue->pop(in_data, in_data_size, file_part, compression_type, ref_id);
				_bz_stram.avail_in = (uint32)in_data_size;
				_bz_stram.next_in = (char*)in_data;
			}
...
void CWStatsFastqReader::operator()()
{
	...
	else
	{
		...
		ReadType read_type;
    uint64 refid_nouse;
		while (fqr->GetPartNew(part, part_filled, read_type, refid_nouse) && !finished)
		{
