...
class CBinaryPackQueue
{
	...
	std::queue<tuple<uchar*, uint64, FilePart, CompressionType, uint64>> q;
	std::mutex mtx;
	std::condition_variable cv_pop, cv_push;
	bool completed = false;
	bool stop = false;
public:
	bool push(uchar* data, uint64 size, FilePart file_part, CompressionType mode, uint64 id)
	{
		std::lock_guard<std::mutex> lck(mtx);
		if (stop)
			return false;
		q.emplace(data, size, file_part, mode, id);
	...
	bool pop(uchar* &data, uint64 &size, FilePart &file_part, CompressionType &mode, uint64& id)
	{
...
class CPartQueue {
	...
	typedef tuple<uchar *, uint64, ReadType, uint64> elem_t;
	typedef queue<elem_t, list<elem_t>> queue_t;
...
public:
	...
	void push(uint64 ref_id, uchar *part, uint64 size, ReadType read_type) {
		unique_lock<mutex> lck(mtx);
		
		bool was_empty = q.empty();
		q.push(make_tuple(part, size, read_type, ref_id));

		if(was_empty)
			cv_queue_empty.notify_all();
	}

	bool pop(uint64 &ref_id, uchar *&part, uint64 &size, ReadType& read_type) {
		unique_lock<mutex> lck(mtx);
		cv_queue_empty.wait(lck, [this]{return !this->q.empty() || !this->n_readers;}); 

		if(q.empty())
			return false;
		std::tie(part, size, read_type, ref_id) = q.front();
		q.pop();
